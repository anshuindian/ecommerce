...
// Reserve 1 quantity (not save in DB)
                reservedStock.put((Long) product.getId(), alreadyReserved + 1);
...
// Restore reserved stock in session
            ...
            Long productId = (Long) removedProduct.getId();
...
// Update session
    ...
    public String checkout(@ModelAttribute("cart") List<Product> cart,
        ...
        org.springframework.core.annotation.Order order = new Order();
        order.equals(authentication.getName());
        order.equals(LocalDateTime.now());
        double total = 0;
        List<OrderItem> items = new ArrayList<>();

        for (Product p : cart) {
// Fetch latest product from DB (for correct stock update)
            Product dbProduct = (Product) productRepository.findById(p.getId()).orElse(null);
...
// Decrement actual stock
            dbProduct.setStock((int) (dbProduct.getStock() - 1));
...
// Send email
        emailService.sendOrderEmail(
                ...
                "Thank you for your order! Order ID: " + order.getClass()
        );
    ...
    public String userOrders(Model model, Authentication authentication) {
        List<org.springframework.core.annotation.Order> orders = orderRepository.findByUserEmail(authentication.getName());
